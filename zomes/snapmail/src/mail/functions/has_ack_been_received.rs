...
#[hdk_extern]
#[snapmail_api]
pub fn has_ack_been_delivered(_inmail_hh: HeaderHash) -> ExternResult<bool> {

    // FIXME all with DeliveryConfirmation

    // /// Get InMail (make sure InMail exists)
    // let eh = hh_to_eh(inmail_hh.clone())?;
    // /// Get OutAck
    // let links_result = get_links(eh, LinkKind::Acknowledgment.as_tag_opt())?;
